...
def analytic_orbit(p, v, n):
    G = 4*(np.pi**2)
    for i in range(n):
        if i != 0:
            Lhat = np.cross(p[i], v[i])
            L = np.linalg.norm(Lhat)
            Lhat /= L        
            E = 0.5 * np.linalg.norm(v[i])**2 - G / np.linalg.norm(p[i])
            a = - 0.5 * G / E
            ecc = (1 + 2 * E * L**2 / G**2)**0.5
            r0 = np.linalg.norm(p[i])
            rhat0 = p[i] / r0
            ...
            for j in range(0,360, 1):
                t = j*np.pi/180
                rx[i].append(r[i]*np.cos(t))
                ry[i].append(r[i]*np.sin(t))
